...
* 継承とオーバーライドを使うと、基底クラスに過剰に依存してしまってよくない (?) みたいな論もあるので、`trait`で振る舞いだけを定義する形はいいところなのかも
  * なんとなく、`interface`だけあるOOPみたいなもんなのかしら
* `trait object`とジェネリクスの違いが、The Rust Programming Language 日本語版の記載だけ見ててもイマイチわからんかったけど、ジェネリクスだと、いずれも`Draw`が実装されていたとしても、`Button`と`SelectBox`の両方を配列に入れることができないことになるんだね
* オブジェクト安全性はイマイチ本質わかってないかも
  * コンパイル時に、`Self`なりジェネリクスなりがあると、そのタイミングで分からないから困るってだけの話？
* 例 (`17.3. オブジェクト指向デザインパターンを実装する`) のとこも一通り読んだけど、自分でなんかやってみないとわかんないかも
  * `trait State`なんだねって感じ。トレイトオブジェクトとして扱うから自然っちゃ自然なのか。なんか`trait`って振る舞いの定義をするものだから、オブジェクトっぽすぎる名前だと違和感あるかと思っちゃった。`17.2. トレイトオブジェクトで異なる型の値を許容する`での説明でも`Draw`トレイトを、トレイトオブジェクトとして扱うみたいな流れだったし
  * この辺はやっていって、使うライブラリのコード見つつで慣れていくしかなさそう

### 18. パターンとマッチング

* 基本的なとこはまあこれまでもやってたわなって感じ
* `論駁可能性`って何って思ったら、ディベートとかの文脈で言う`反駁`と似たような意味っぽい。
  * 英語だと`Refutability`
* だから、論駁可能=パターンマッチに失敗しうる、くらいに思えばいいんじゃないかって感じ
* あとはまあ、こういう書き方があるのかって感じだから、使ううちに便利さが分かればいいや
